...
const showInfo = (result) => {
    ...
    document.querySelector('.tempDesc').innerHTML = `${result.description}`;  
    document.querySelector('.ventoPonto').style.transform = `rotate(${result.windAngle-90}deg)`; 
    ...
    document.querySelector('.windDeg>.value').textContent = windDirection + ', ' + result.windSpeed,
    document.querySelector('.humidity>.value').textContent = `Umidade: ${result.humidity}%`,
    document.querySelector('.pressure>.value').textContent = `Presão: ${result.pressure}hPa`,
